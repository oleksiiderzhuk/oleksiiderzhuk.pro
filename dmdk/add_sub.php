...
$regno_check = False;
$tax_check = False;
$sql = "SELECT count(*) FROM person where regno = '$regno' and state = 1";
	$result = mysqli_query($link, $sql);
	$regno_count = mysqli_fetch_array($result)['count(*)'];
if ($regno_count > 0){
$regno_check = True;	
if($regno_check==True){
	header("Location: regno_error.php");
}
}

elseif(check_value('person','tax_code', $tax_code)==1){
	$tax_check = True;
	if($tax_check==True){
