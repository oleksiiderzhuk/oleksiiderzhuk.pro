...
$sql = "SELECT * FROM adress WHERE person_id = $person_id and start_date like '$today%' and current = '1'";
...
<body>
	<form method="post" style="margin-left: 200px">
		...
		<div style="margin-left: 66px; margin-top: 34px">
			...
			<p style="text-indent: 45px;  text-align: justify; font-family: serif; font-size: 14pt">За результатами розгляду матеріалів, наданих <?=date("d.m.Y", strtotime($income_date))?> № <?=$income_number?>, повідомляється, що до видів операцій з дорогоцінними металами і дорогоцінним камінням та місця провадження діяльності <u><?=$opf_alias?> <?=$name?></u> внесено такі зміни до реєстру суб&rsquo;єктів господарювання, які здійснюють операції з дорогоцінними металами і дорогоцінним камінням.</p>
			
			
			<table class="activity" cellpadding="5px"  cellspacing=0 border=1 width=100%>
			<tr>
				<th style="font-family: serif; font-size: 14pt; font-weight: normal; " width=38%>Місце провадження діяльності суб'єкта господарювання</th>
			...
			$today=date('Y-m-d');
			//--------works before
			$sql="SELECT * FROM adress WHERE person_id=$person_id and worktype not like '0' and end_date like '$today%' and current = '0' and isvalid = '1'";

			$all_adress_before = mysqli_query($link, $sql);
			foreach($all_adress_before as $value):
				
				$before_worktype = explode(",", $value['worktype']);
...
<?php
			...
			foreach($all_adress_today as $value):
				...
				$diff=array();
				$diff = array_intersect($before_worktype, $worktype); 
				for ($i=0; $i<count($worktype); $i++) {
				if (!in_array($worktype[$i], $before_worktype)) {
					$new_worktypes[]=$worktype[$i];
				}
				}
				...
				?>
				<tr>
					<th style="font-weight: normal; text-align: center; vertical-align: top; font-family: serif; font-size: 14pt; padding: 5px"><?=$adr;?></th>
					<th style="font-weight: normal; text-align: justify; vertical-align: top; font-family: serif; font-size: 14pt; padding: 5px">
						...
						foreach($worktype as $t_value){
							$i++;
							if ($i<$worktype_count) {
								echo show_work_name($t_value).';<br>';
							}
							else {
						 		echo show_work_name($t_value).'<br>';
							}	
