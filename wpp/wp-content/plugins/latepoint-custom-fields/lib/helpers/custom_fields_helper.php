...
class OsCustomFieldsHelper {
  ...
  public static function prepare_to_save($array_to_filter){
	...
	public static function get_custom_field_types(){
		$custom_field_types = [
			'text' => __('Text Field', 'latepoint-custom-fields'),
			'phone_number' => __('Phone Number Field', 'latepoint-custom-fields'),
			'number' => __('Number Field', 'latepoint-custom-fields'),
			'hidden' => __('Hidden Field', 'latepoint-custom-fields'),
			...
			'google_address_autocomplete' => __('Google Address Autocomplete', 'latepoint-custom-fields'),
			'file_upload' => __('File Upload', 'latepoint-custom-fields')];
	...
	public static function get_custom_field_default_value_html(string $fieldType, string $fieldName, string $existingValue = ''): string {
		$atts = [ 'theme' => 'bordered', 'placeholder' => __('Enter Field Default Value', 'latepoint-custom-fields') ];
	...
	public static function load_countries_list($include_empty = true){
  	$country_codes = [
			'' => __('No Restrictions', 'latepoint-custom-fields'),
			'au' => 'Australia',
      'at' => 'Austria',
      'be' => 'Belgium',
      'br' => 'Brazil',
  ...
  public static function should_step_be_skipped($skip, $step_name, $booking_object){
    if($step_name == 'custom_fields_for_booking'){
      $custom_fields_for_booking = self::get_custom_fields_arr('booking', 'customer', $booking_object);
	 ...
	 * @return string
	 */
	public static function output_custom_fields_for_model($custom_fields, OsModel $model, $fields_for = 'customer'){
		$html = '';
    ...
    foreach($custom_fields as $custom_field){
      ...
      $required_class = ($custom_field['required'] == LATEPOINT_VALUE_ON) ? 'required' : '';
			...
			$name = $fields_for.'[custom_fields]['.$custom_field['id'].']';
      ...
      switch ($custom_field['type']) {
        ...
        case 'hidden':
          ...
          break;
        case 'textarea':
          $html.= OsFormHelper::textarea_field($name, $label, $value, ['class' => $required_class, 'placeholder' => $placeholder, 'theme' => 'simple'], ['class' => $width]);
          break;
        case 'select':
          ...
          break;
        case 'checkbox':
          $html.= OsFormHelper::checkbox_field($name, $label, 'on', ($value == 'on') , ['class' => $required_class], ['class' => $width]);
          break;
    ...
    foreach($fields_rules as $custom_field){
      if($custom_field['required'] != 'on') continue;
	        ...
	        if (empty($fields_data[$custom_field['id']]) || $fields_data[$custom_field['id']] == 'off') {
		        ...
		        $error_message = sprintf(__('%s has to be attached', 'latepoint-custom-fields'), $custom_field['label']);
		        $errors[] = ['type' => 'validation', 'message' => $error_message];
					}else{
						...
						if(!empty($files['name']['custom_fields'][$custom_field['id']])){
					        ...
					        $error_message = sprintf(__('%s file type is not supported.', 'latepoint-custom-fields'), $custom_field['label']);
					        $errors[] = ['type' => 'validation', 'message' => $error_message];
								}
							}
						}
  ...
  public static function get_checkbox_value($value){
    return ($value == 'on') ? __('Yes', 'latepoint-custom-fields') : __('No', 'latepoint-custom-fields');
  }
  ...
  public static function generate_condition_form($custom_field_id, $condition_id = false, $condition_data = false){
    $properties = [ 'service' => __('Service', 'latepoint-custom-fields'), 
                    'agent' => __('Agent', 'latepoint-custom-fields')];
    ...
    $html = '<div class="cf-condition" data-condition-id="'.$condition_id.'">'.
                ...
                OsFormHelper::select_field('custom_fields['.$custom_field_id.'][conditions]['.$condition_id.'][operator]', false, array('equal' => __('is equal to', 'latepoint-custom-fields'), 'not_equal' => __('is not equal to', 'latepoint-custom-fields')), $condition_data['operator'], [], ['class' => 'custom-field-condition-operator-w']).
                ...
                '<div class="custom-field-condition-property-w" 
                      ...
                      data-os-after-call="latepointCustomFieldsAdminAddon.add_custom_field_condition"><button class="latepoint-btn-outline latepoint-btn"><i class="latepoint-icon latepoint-icon-plus2"></i><span>'.__('AND', 'latepoint-custom-fields').'</span></button></div>'.
            '</div>';
    return $html;
  }
